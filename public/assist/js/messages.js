...
function addDataToTable(doc, i) {
    ...
    td3.textContent = doc.data().message;
...
firebase.firestore().collection("contact").get().then(function (querySnapshot) {
    "use strict";
...
$("#tbshow").on("click", function (e) {
    ...
    if (docID !== null) {
        firebase.firestore().collection("contact").doc(docID).delete().then(function () {
            window.alert("Delete Success");
            window.location.assign("conmesantactdsages");
        }).catch(function (error) {
